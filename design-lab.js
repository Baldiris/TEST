...
function drawMap(){
 const first=selected&&!arrived?shortest(position,selected):{nodes:[position],edges:[],distance:0};
 const onward=shortest(selected&&!arrived?selected:position,goal);
 const nodes=[...first.nodes,...onward.nodes.slice(1)];
 const rows=Math.max(2,Math.ceil(nodes.length/4)), height=rows*140+80;
 const points=nodes.map((name,i)=>{const row=Math.floor(i/4),col=i%4;return {name,x:row%2?110+col*190:680-col*190,y:height-80-row*140}});
 const svg=$('journey-map');svg.setAttribute('viewBox',`0 0 800 ${height+40}`);
 svg.setAttribute('aria-label',nodes.join('、')+'。選択駅まで'+first.distance+'マス、その先ゴールまで'+onward.distance+'マス');
 ...
 points.forEach((p,i)=>{
  ...
  const transfer=incoming&&outgoing&&incoming.line!==outgoing.line;
  ...
  const label=isCurrent?'現在地':isChoice?'選択中':isGoal?'GOAL':transfer?'乗換 '+incoming.line+' → '+outgoing.line:'';
...
function render(){
 ...
 candidates.forEach(([name,route],i)=>{
 ...
 if(selected&&!arrived){
  const route=shortest(position,selected);
  $('route-detail').innerHTML=`<strong>${route.distance} マス / ${[...new Set(route.edges.map(e=>LINES[e.line].name))].join(' → ')}</strong><br>${route.nodes.join(' → ')}`;
...
$('mobile-preview').onclick=()=>{const active=document.body.classList.toggle('preview');$('mobile-preview').setAttribute('aria-pressed',String(active));$('mobile-preview').textContent=active?'通常幅に戻す':'スマホ幅で試す'};
render();
